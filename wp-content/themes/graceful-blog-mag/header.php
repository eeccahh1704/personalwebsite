...
<body <?php body_class(); ?>>
	...
	<a class="skip-link screen-reader-text" href="#primary"><?php esc_html_e( 'Skip to content', 'graceful-blog-mag' ); ?></a>	
	...
	<div id="site-container">
		...
		<?php
			if ( is_home() && !is_paged() ) {
			    ...
			    $graceful_popular_post_enabled = graceful_blog_mag_options('popular_post_ticker_show');
			    // Show Post Ticker if enabled
			    if ( $graceful_popular_post_enabled ) {
			        get_template_part('template-parts/sections/popular', 'posts');
			    }

			    // Show Featured Boxes if enabled
			    $graceful_featured_boxes_enabled = graceful_blog_mag_options('featured_boxes_show');
